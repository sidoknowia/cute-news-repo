...
function hash_generate($password)
{
    ...
    (
        0 => md5($password),
        1 => SHA256::hash($password . $cfg['crypt_salt']),
    );

    if ( function_exists('hash') && HASH_METHOD != 'sha256' ) $try[] = hash( HASH_METHOD, $password . $cfg['crypt_salt'] );
...
function hesc($html)
{
    ...
    {
        $allowed_tags = explode(',', 'a,i,b,u,p,h1,h2,h3,h4,h5,h6,hr,ul,ol,br,li,tr,th,td,tt,sub,sup,img,big,div,code,span,abbr,code,acronym,address,blockquote,center,strike,strong,table,thead');
...
class SHA256 extends hash
{
    ...
    static function hashHex(&$hashData)
    {
        ...
        reset($hashData->hash);
        do { $str .= sprintf('%08x', current($hashData->hash)); } while(next($hashData->hash));
        return $str;
    }
    ...
    static function hashBin(&$hashData)
    {
        $str = '';
        reset($hashData->hash);
        do { $str .= pack('N', current($hashData->hash)); } while(next($hashData->hash));
        return $str;
    }
}
