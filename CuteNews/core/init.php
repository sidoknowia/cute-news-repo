<?php
    ...
    // -----------------------------------------------------------------------------------------------------------------
    if (isset($HTTP_SESSION_VARS))  extract($HTTP_SESSION_VARS,     EXTR_SKIP);
    if (isset($_SESSION))           extract($_SESSION,              EXTR_SKIP);
    if (isset($HTTP_COOKIE_VARS))   extract($HTTP_COOKIE_VARS,      EXTR_SKIP);
    if (isset($_COOKIE))            extract($_COOKIE,               EXTR_SKIP);
    if (isset($HTTP_POST_VARS))     extract($HTTP_POST_VARS,        EXTR_SKIP);
    if (isset($_POST))              extract($_POST,                 EXTR_SKIP);
    if (isset($HTTP_GET_VARS))      extract($HTTP_GET_VARS,         EXTR_SKIP);
    if (isset($_GET))               extract($_GET,                  EXTR_SKIP);
    if (isset($HTTP_ENV_VARS))      extract($HTTP_ENV_VARS,         EXTR_SKIP);
    ...
    if (is_array($category)) foreach ($category as $k => $v) $category[$k] = htmlspecialchars($v);
