...
{
    if (file_exists(SERVDIR.'/cdata/htpath.php'))
    {
        include (SERVDIR.'/cdata/htpath.php');

        // Do Save path
        if ($do == 'submit' || $do == 'build')
        {
            $w = fopen( $ht_path.'/.htaccess', 'w' );
            fwrite($w, $_POST['mod_rewrite']);
            fclose($w);
        }

        if (!file_exists($ht_path.'/.htaccess')) $mod_rewrite = false;
        else $mod_rewrite = join('', file($ht_path.'/.htaccess'));

        if ($do == 'build')
        {
            $additions = '';
            $start = intval( stripos($mod_rewrite, '# CUTENEWS REWRITES #') );
            $rewon = stripos($mod_rewrite, 'RewriteEngine On');

            // Enable RewriteEngine if no present before
            if ($rewon >  $start || $rewon === false) $additions = "RewriteEngine On\n";

            $w = fopen($ht_path.'/.htaccess', 'w');
            fwrite($w, substr($mod_rewrite, 0, $start));
            fwrite($w, "# CUTENEWS REWRITES #\n$additions");

            // Write basic rules
            fwrite($w, read_tpl('ht_basic'));

            fclose($w);

            // Reload
            $mod_rewrite = join('', file($ht_path.'/.htaccess'));
        }

        echoheader("wizard", lang("MOD Rewrite .htaccess confure"));
        echo proc_tpl('wizard/modrewrite', array
        (
            'htpath' => $ht_path,
            'mod_rewrite' => htmlspecialchars($mod_rewrite)
        ));
