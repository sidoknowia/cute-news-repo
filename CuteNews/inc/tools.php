...
elseif ($action == 'do_update' )
{
    ...
    else
    {
        ...
        while (time() - $mc < 15 && $proc < count($bundle))
        {
            ...
            if ( preg_match('~\.[a-z0-9]+?$~i', $dirs[count($dirs)-1] ) )
            {
                // Make dirs...
            ...
            elseif (preg_match('~Exported revision (\d+)~i', $bundle[$proc], $rev))
            {
        ...
        echo ( '<html>
                <head><meta http-equiv="refresh" content="1; URL='.$config_http_script_dir.'?mod=tools&action=do_update&proc='.$proc.'"></head>
